...
function showNearest(latitude, longitude){
	...
	for (let i=0; i<5; i++) {
		...
		let lat = castlesArray[i].pos[0];
		let lon = castlesArray[i].pos[1];
		outputHTML += `<div class="castleItem" onclick="updateMap(${lat}, ${lon})">
										<div>
											<img class="thumb" src="images/thumbs/${castlesArray[i]["img"]}">
										</div>
										<div class="castleInfo">
											<h5 style="font-weight: bold;">${castlesArray[i]["name"]}</h5>
											<h5>${castlesArray[i]["built"]}</h5>
